...
namespace Framework
{
    public class Security
    {
        ...
        public void Recv(byte[] buffer, int offset, int length)
        {
            ...
            //sw.Stop();
            //Utils.WriteLine($"{sw.ElapsedMilliseconds} ms");
        }
        ...
        public void Recv(TransferBuffer raw_buffer)
        {
            ...
            lock (m_class_lock)
            {
                ...
                while (length > 0)
                {
                    ...
                    raw_buffer.Buffer.CopyTo(m_recv_buffer.Buffer, raw_buffer.Offset + index);
                    ...
                    while (m_recv_buffer.Size > 0)
                    {
                        ...
                        //Array.Copy(m_recv_buffer.Buffer, 0, m_current_buffer.Buffer, m_current_buffer.Offset, max_copy_count);
                        ...
                        // If there is data reamain in the buffer, copy it over the data
                        ...
                        if (m_recv_buffer.Size > 0)
                        {
                            ...
                            //m_recv_buffer.Buffer.CopyTo(m_recv_buffer.Buffer, max_copy_count);
                        }
                        ...
                        if (m_client_security)
                        {
                            if (m_security_flags.security_bytes == 1)
                            {
                                ...
                                if (packet_security_count != expected_count)
                                {
                                    ...
                                    {
                                        UTILS.WriteLine($"Count byte mismatch: [0x{packet_opcode:X4}:{packet_security_count}/{expected_count}]");
                                    }
                                ...
                                if (packet_security_crc != expected_crc)
                                {
                                    if (Settings.LOG_PROXY_ERRORS)
                                    {
                                        UTILS.WriteLine($"CRC byte mismatch: [0x{packet_opcode:X4}:{packet_security_crc}/{expected_crc}]");
                                    }
